...
def generate_page(from_path, template_path, dest_path):
    ...
    finalhtml = tempelate.replace("{{ Content }}", htmlstring)

    with open(dest_path, "w") as f:
        print(f"writing html to  {dest_path}")
        f.write(finalhtml)

def generate_pages_recursive(dir_path_content, tempelate_path, dest_dir_path):
    ...
    for file in list_of_files:
        ...
        if os.path.isfile(current_file):
            ...
            generate_page(current_file, tempelate_path, dest_path)
        else:
            generate_pages_recursive(current_file, tempelate_path, dest_path)
...
def main():
    ...
    recurse_and_copy(dir_path_static, dir_path_public)
